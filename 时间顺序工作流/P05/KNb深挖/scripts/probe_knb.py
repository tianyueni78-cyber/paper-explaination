...
def exploration_channel(fixed,step):return (fixed+(step if step<12 else (step-12)//4))%12
...
def walk(data,parent,schedule,policy,fixed,budget,seed):
    ...
    while total<budget and len(trace)<96:
        ...
        if policy=='round':channel=(fixed+step)%12
            ...
            if step<12 or step%4==0:channel=exploration_channel(fixed,step)
